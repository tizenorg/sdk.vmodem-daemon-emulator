...
int g_sim_sec_general_response_error = 0x8000;
...
int state_sim_change_password(char *password,int type,int length)
{
	...
	case GSM_SIM_PIN_REQ_STATE:
		...
		db_sim_sec_edit(DB_PIN ,g_pin_value,0);
		return 1;
	    }
	...
	case GSM_SIM_PIN2_REQ:
		...
		db_sim_sec_edit(DB_PIN2 ,g_pin2_value,0);
		return 1;
	    }
	...
	case GSM_SIM_PUK_REQ:
		...
		db_sim_sec_edit(DB_PUK ,g_puk_value,0);
		return 1;
	    }
	...
	case GSM_SIM_PUK2_REQ:
		...
		db_sim_sec_edit(DB_PUK2 ,g_puk2_value,0);
		return 1;
	    }
...
int state_sim_verify_password(char *password,int type,int length)
{
	...
	case GSM_SIM_PIN_REQ_STATE:
		...
		if(!memcmp(g_pin_value,password,length))
		{
		    g_pin_cnt	=	3;
...
char state_sim_status_mgr(int type)
{
	...
	case GSM_SIM_PIN_REQ_STATE:
		...
		if(g_pin_cnt == 0 )
		    state_sim_set_sec_status(GSM_SIM_PUK_REQ);
	    }
	    break;
	...
	case GSM_SIM_PIN2_REQ:
		...
		if(g_pin2_cnt == 0 )
		    state_sim_set_sec2_status(GSM_SIM_PUK2_REQ);
	    }
	    break;
	...
	case GSM_SIM_PUK_REQ:
		...
		if(g_puk_cnt == 0 )
		    state_sim_set_sec_status(GSM_SIM_FRIZEN);
	    }
	    break;
	...
	case GSM_SIM_PUK2_REQ:
		...
		if(g_puk2_cnt == 0 )
		    state_sim_set_sec2_status(GSM_SIM_FRIZEN);
	    }
	    break;
...
int state_sim_set_sec_status(int status)
{
    ...
    db_sim_sec_edit(DB_SIM_STATE, NULL,status);
...
int state_sim_set_sec2_status(int status)
{
    ...
    db_sim_sec_edit(DB_LOCK_SC2_STATE, NULL,status);
...
void set_sim_sec_general_response_error( int error )
{
    ...
    g_sim_sec_general_response_error = error;
}
...
int get_sim_sec_general_response_error()
{
    ...
    return g_sim_sec_general_response_error;
}
